...
```{r}
library(usethis)
use_git_config(user.name = "birrermi", user.email = "birrermi@student.ethz.ch")
```
...
```{r}
sanitation_Algeria_2000_2020_not_san_sm <- sanitation |> 
  filter(year == 2000 | year == 2020, name == "Algeria", varname_short != "san_sm")
```
...
```{r}
sanitation_Algeria_2000_2020_not_san_sm |> 
  count(year)
```
...
```{r}
ggplot(data=sanitation_Algeria_2000_2020_not_san_sm,
       aes(x=residence, 
           y=percent,
           fill = varname_long)) +
  geom_col() +
...
1.  Look at the plot that you created. What do you notice about the order of the bars / order of the legend?
    ...
    *there are a lot of basic sanitation services*

2.  What would you want to change?

    *get everything at least to basic sanitation services*

3.  Why did we remove "safely managed sanitation services" from the data set in Task 3?

    *safely managed sanitation services are good to have and no problem. To see how serious the problem is one should not have a look at safely managed sanitation services*
...
2.  What do you observe when you look at the code and plot?
    ...
    *The plot shows the percentages of the different residence types for different types of sanitation services in the year 2020.*
