...
3.  How many variables are in the data set?
    ...
    *344x8=2752 variables*

4.  How many observations are in the data set?

    *8(species, island, bill_length_mm,..., year)*
...
-   Numerical:
    ...
    -   discrete variables: year/ flipper_length_mm / body_mass_g /

    -   continous variable: bill_length_mm / bill_depth_mm
...
```{r}
penguins_body_mass_lighter_2900g <- penguins |> 
...
```{r}
penguins_adelie_bill_length_greater_40mm <- penguins |> 
...
```{r}
penguins_gentoo_bill_depth_greater_equal_15.5mm <- penguins |> 
...
```{r}
penguins_male_dream_biscoe <- penguins |> 
  filter(sex == "male" & island != "Torgersen")
...
```{r}
penguins_female_Adelie_bill_lenght_less_35mm <- penguins |> 
...
```{r}
penguins_female_chinstrap_dream_torgersen <- penguins |> 
  filter(sex == "female" & species == "Chinstrap" & island != "Biscoe")
```
...
```{r}
penguins_gentoos_or_bodymass_greater_4500g <- penguins |> 
...
*na.rm ignores the NA values*

```{r}
penguins |> 
  group_by(species) |> 
  summarise(flipper_max = max(flipper_length_mm, na.rm = TRUE),
...
```{r}
penguins_female_torgersen_biscoe <- penguins |> 
  filter(sex == "female", island != "Dream") |> 
  count(sex, island)
penguins_female_torgersen_biscoe
```
